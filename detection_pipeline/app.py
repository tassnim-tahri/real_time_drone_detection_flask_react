        ...
        if _COSMOS_MODEL is None or _COSMOS_PROCESSOR is None:
            try:
                ...
                _COSMOS_MODEL = AutoModelForVision2Seq.from_pretrained(
                    ...
                    offload_folder="offload_cache",   # spills overflow to disk
...
def build_langchain_chain(llm_provider: str, llm_model: str, groq_api_key: "REMOVED") -> Any:
...
def main() -> None:
    ...
    if not video_path.exists():
        raise FileNotFoundError(f"Video not found: {video_path}")
    args.llm_provider="groq"
    args.llm_model = "llama-3.3-70b-versatile"
